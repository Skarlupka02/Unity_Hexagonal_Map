...
public class HexGridChunk : MonoBehaviour
{
    ...
    void Triangulate(HexDirection direction, HexCell cell)
    {
        ...
        else
        {
            ...
            if (!cell.IsUnderwater && !cell.HasRoadThroughEdge(direction)) features.AddFeature(cell, (center + e.v1 + e.v5) * (1f / 3f));
        }
    ...
    Vector2 GetRoadInterpilators(HexDirection direction, HexCell cell)
    {
    ...
    void TriangulateRoadAdjacentToRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        else if (cell.IncomingRiver == cell.OutgoingRiver.Opposite())
        {
            ...
            if (previousHasRiver)
            {
                if (!hasRoadTroughEdge && !cell.HasRoadThroughEdge(direction.Next()))
                {
                    return;
                }
            ...
            {
                if (!hasRoadTroughEdge && !cell.HasRoadThroughEdge(direction.Previous()))
                {
                    return;
                }
    ...
    {
        if (meshFilter == null) meshFilter = terrain.GetComponent<MeshFilter>(); 

        if (GetComponent<LineRenderer>() == null) lineRenderer = gameObject.AddComponent<LineRenderer>();

        if (lineRenderer == null) lineRenderer = terrain.GetComponent<LineRenderer>();

        terrain.DisplayEdges1(meshFilter, lineRenderer);
        //DisplayEdges2();
    }
